...
def replace_values_to_generator_circuit(input_file_path, output_file_path, p, r, have_stabilizer, nkd, measurement_param):
    ...
    if m_pattern in content:
        ...
        # 替换前面所有的M(0.005)
        if measurement_param == 0:
            before_last = before_last.replace('M(0.005)', f'M')
        else:
            before_last = before_last.replace('M(0.005)', f'M({p*measurement_param})')
                    ...
                    for r in [1,d]:
                        for noise_model in noise_models:
                            for measurement_param in measurement_params:
                                ...
                                if have_stabilizer:
                                    output_file_path = os.path.join(current_file_directory, folder, f"nkd_{nkd[0]}_{nkd[1]}_{nkd[2]}_r{r}", f"circuit_noisy_{noise_model}_p{p}_m{measurement_param}.stim")
                                else:
                                    output_file_path = os.path.join(current_file_directory, folder, f"nkd_{nkd[0]}_{nkd[1]}_{nkd[2]}_r{r}", f"circuit_noisy_{noise_model}_p{p}_m{measurement_param}_no_stabilizer.stim")
                                ...
                                replace_values_to_generator_circuit(input_file_path, output_file_path, p_value, r, have_stabilizer, nkd, measurement_param)
