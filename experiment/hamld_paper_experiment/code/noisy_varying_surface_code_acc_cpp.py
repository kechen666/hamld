...
logger = setup_logger("epmld_paper_experiment/noisy_varying_surface_code_acc_cpp", log_level=logging.INFO)
...
def main():
    ...
    output_file = os.path.join(output_dir, f"noisy_varying_surface_code_acc_cpp_results_{timestamp}.csv")
    ...
    decoder_methods = ["MWPM", "HAMLD_cpp"]
    ...
    # 创建存储结果的CSV文件并写入表头
    fieldnames = ['code_task', 'd', 'round', 'probability', 'noise_model', 'decoder_method', 'logical_error_rate', 'have_stabilizer', 'total_running_time']
    # 如果文件不存在，写入表头
...
# 后台运行命令
# nohup python /home/normaluser/ck/epmld/experiment/epmld_paper_experiment/code/noisy_varying_surface_code_acc_cpp.py > /home/normaluser/ck/epmld/experiment/epmld_paper_experiment/log/noisy_varying_surface_code_acc_cpp_output.log 2>&1 &
# 528333