...
logger = setup_logger("epmld_paper_experiment/approx_param_varying_surface_code_speed_cpp", log_level=logging.INFO)
...
def main():
    ...
    output_file = os.path.join(output_dir, f"approx_param_varying_surface_code_speed_cpp_results_{timestamp}.csv")
    ...
    approximate_params = [10, 100, 200]
    prioritys = [1, 0, 0, -1]
    priority_topks = [10, 100, 150, 200]

    # 创建存储结果的CSV文件并写入表头
    fieldnames = ['code_task', 'd', 'round', 'approximate_param', 'priority', 'priority_topk', 'decoder_method', 'average_per_round_time', 'have_stabilizer']
    # 如果文件不存在，写入表头
...
# 后台运行命令
# nohup python /home/normaluser/ck/epmld/experiment/epmld_paper_experiment/code/approx_param_varying_surface_code_speed_cpp.py > /home/normaluser/ck/epmld/experiment/epmld_paper_experiment/log/approx_param_varying_surface_code_speed_cpp_output.log 2>&1 &
# 3312387