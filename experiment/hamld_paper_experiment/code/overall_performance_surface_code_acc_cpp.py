...
logger = setup_logger("epmld_paper_experiment/overall_performance_surface_code_acc_cpp", log_level=logging.INFO)
...
def main():
    ...
    output_file = os.path.join(output_dir, f"overall_performance_surface_code_acc_cpp_results_{timestamp}.csv")
                        ...
                        for r in rounds:
                            for noise_model in noise_models:
                                ...
                                #     for approximate_param in approximate_params:
                                        if decoder_method == "MLD" or decoder_method == "EMLD":
                                            # 对于d>5 并且r>1的情况下，进行跳过。
                                            if d >= 5 and r > 1:
                                                logger.info("MLD方法只测试d=3，r=3，以及r=1的情况。")
                                                continue
                                        #     continue
                                        ...
                                        # approximate_param = max(int(detector_number/8), d)
                                        # priority_topk = max(int(detector_number/6), d)
                                        # approximate_param = priority_topk = max(int(detector_number/2), d)
                                        # approximate_param = priority_topk = max(int(detector_number/4), d)
                                        approximate_param = priority_topk = int(detector_number/4)
...
# 后台运行命令
# nohup python /home/normaluser/ck/epmld/experiment/epmld_paper_experiment/code/overall_performance_surface_code_acc_cpp.py > /home/normaluser/ck/epmld/experiment/epmld_paper_experiment/log/overall_performance_surface_code_acc_cpp_output.log 2>&1 &
# 571591