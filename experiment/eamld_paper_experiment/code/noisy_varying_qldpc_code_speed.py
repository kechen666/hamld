...
from eamld.benchmark import generate_qldpc_detector_error_model, DecoderSpeedBenchmark
# 设置 logging 配置，放在模块级别
from eamld.logging_config import setup_logger

logger = setup_logger("eamld_paper_experiment/noisy_varying_qldpc_code_speed", log_level=logging.INFO)

def main():
    # 设置输入数据的相关路径
    related_path = "/home/normaluser/ck/eamld/data/external/eamld_experiment_data/eamld_paper_experiment/noise_varying/qldpc_code"

    # 设置输出目录
    output_dir="/home/normaluser/ck/eamld/experiment/eamld_paper_experiment/result"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join(output_dir, f"noisy_varying_qldpc_code_speed_results_{timestamp}.csv")
    ...
    # 设置待测试的解码方法：
    # decoder_methods = ["BP+OSD", "EAMLD"]  # BP+OSD与近似的MLD方法（EAMLD）的对比。
    ...
    # 创建存储结果的CSV文件并写入表头
    fieldnames = ['code_task', 'nkd', 'round', 'probability', 'noise_model', 'decoder_method', 'average_per_round_time', 'have_stabilizer']
    # 如果文件不存在，写入表头
...
# 后台运行命令
# nohup python /home/normaluser/ck/eamld/experiment/eamld_paper_experiment/code/noisy_varying_qldpc_code_speed.py > /home/normaluser/ck/eamld/experiment/eamld_paper_experiment/log/noisy_varying_qldpc_code_speed_output.log 2>&1 &
# 2410396