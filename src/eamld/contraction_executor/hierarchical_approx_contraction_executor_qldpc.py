...
logger = setup_logger("contraction_executor/hierarchical_approx_contraction_executor_qldpc", log_level=logging.WARNING)
...
class HierarchicalApproximateContractionExecutorQldpc:
    def __init__(self, detector_number: int, logical_number: int, order: List[str],
        ...
        self.contract_no_flipped_detector = contract_no_flipped_detector
        self.change_approximate_param = change_approximate_param
    ...
    def _first_level_contraction_strategy(self):
        # 获取包含所有翻转检测器的子超图
    ...
    def _second_level_contraction_strategy(self):
        # 构建超边缓存
        # 获取包含所有翻转检测器的子超图
        remove_sub_hypergraph = self.hypergraph.remove_nodes(nodes = self.flipped_detector_set)
        
        # 从子超图中获取原始超边权重字典
        original_dict = remove_sub_hypergraph.get_hyperedges_weights_dict()
    ...
    def approximate_distribution(self, updated_prob_dist: Dict[str, Union[np.float32, np.float64]],) -> Dict[str, Union[np.float32, np.float64]]:
        ...
        """
        if self.change_approximate_param:
            approximate_param = self.approximate_param * len(self.flipped_detector_set)
        else:
    ...
    def mld_contraction_no_slicing(self, syndrome: np.ndarray[bool]) -> Dict[str, Union[np.float32, np.float64]]:
        ...
        if len(self.flipped_detector_set) == 0:
            ...
            sub_contractable_hyperedges_weights_dict, self.order = self._first_level_contraction_strategy()
            ...
            if self.contract_no_flipped_detector:
                sub_contractable_hyperedges_weights_dict, self.order = self._second_level_contraction_strategy()
                self.hyperedge_cache, self.relevant_hyperedge_cache = build_hyperedge_contraction_caches(sub_contractable_hyperedges_weights_dict, self.order)
                
                prob_dist, _ = self.single_node_online_mld_contraction(
                    syndrome = syndrome,
                    init_prob_dist = prob_dist,
                    init_contractable_hyperedges_weights = sub_contractable_hyperedges_weights_dict
                )
