...
logger = setup_logger("contraction_executor/contraction_executor_int", log_level=logging.WARNING)

def int_to_binary_bitwise(n: int, length: int) -> Tuple[bool, ...]:
    # 生成布尔数组
    start = length - 1
    stop = -1
    step = -1
    return tuple((n >> i) & 1 == 1 for i in range(start, stop, step))

def binary_to_int(binary: np.ndarray | Tuple[bool]) -> int:
    # 如果是 np.ndarray 类型，直接操作它
    if isinstance(binary, np.ndarray):
        return np.dot(binary.astype(int), 1 << np.arange(len(binary)-1, -1, -1))  # 位运算法
    # 如果是 tuple 类型，先转为 np.ndarray 处理
    elif isinstance(binary, tuple):
        binary_array = np.array(binary, dtype=int)
        return np.dot(binary_array, 1 << np.arange(len(binary_array)-1, -1, -1))  # 位运算法
    else:
        raise ValueError("Unsupported type for binary_to_int. Expected np.ndarray or Tuple[bool].")

def check_bit_value(num:int, position: int, bit_length: int, expected_value: int):
    """
    此函数用于检测整数 num 的指定位是否等于预期值。
    :param num: 要检测的整数
    :param position: 要检测的位的位置，从 0 开始计数（从左到右）
    :param bit_length: 整数的位数
    :param expected_value: 待检测的值，应为 0 或 1
    :return: 如果指定位等于预期值，返回 True；否则返回 False
    """
    # 确保预期值是 0 或 1
    position = bit_length - position - 1
    assert expected_value in (0, 1), "预期值必须是 0 或 1。"
    return ((num >> position) & 1) == expected_value

class ContractionExecutorInt:
    def __init__(self, detector_number: int, logical_number: int, order: List[str], sliced_hyperedges: List[str], contractable_hyperedges_weights_dict: Dict[str, Union[np.float32, np.float64]], accuracy: str = "float64"):
        """
        ...
        Args:
            ...
            contractable_hyperedges_weights_dict (Dict[str, Union[np.float32, np.float64]]): The weights associated with each hyperedge to contract.
    ...
    def flip_bits(self, binary_int: int, hyperedge: str) -> int:
        """
        Flip the specified bits in a binary integer based on the hyperedge.

        Args:
            binary_int (int): The binary integer. D0对应最高位， D1对应次高位，依此类推，直到L0，同时L_n为最低位。
        ...
        Returns:
            int: The binary integer after flipping the specified bits.
        ...
        for bit_label in hyperedge_list:
            if bit_label[0] == 'D':
                 # 根据 D 后面的数字计算从最高位开始的索引
                index = self.total_length - 1 - int(bit_label[1:])
            elif bit_label[0] == 'L':
                index = self.logical_number - 1 - int(bit_label[1:])
            else:
    ...
    def get_task_initial_input(self) -> Tuple[Dict[int, Union[np.float32, np.float64]], Dict[str, Union[np.float32, np.float64]]]:
        """
        ...
        Returns:
            Tuple[Dict[int, Union[np.float32, np.float64]], Dict[str, Union[np.float32, np.float64]]]:
                - Initial probability distribution.
                - Dictionary of contractable hyperedge weights.
        """
        init_prob_dist: Dict[int, Union[np.float32, np.float64]] = {}
    ...
    def get_parallel_task_initial_input(self) -> Tuple[Dict[int, Union[np.float32, np.float64]],
    ...
    def contract_hyperedge(
        self,
        prob_dist: Dict[int, Union[np.float32, np.float64]],
        contractable_hyperedges_weights_dict: Dict[str, Union[np.float32, np.float64]],
        ...
        Args:
            prob_dist (Dict[int, Union[np.float32, np.float64]]): Current probability distribution.
        ...
        non_flip_contracted_hyperedge_prob = (self._to_numeric_type(1) - contracted_hyperedge_prob)
        
        # Create a defaultdict to store the updated probability distribution (default to 0.0)
        # updated_prob_dist = defaultdict(self.accuracy_type())
        updated_prob_dist = {}

        # Iterate over the current probability distribution
        for binary_int, prob in prob_dist.items():
            # Flip the bits for the current hyperedge
            flipped_int = self.flip_bits(binary_int, contracted_hyperedge)
            ...
            if binary_int in updated_prob_dist:
                updated_prob_dist[binary_int] += non_flipped_prob
            else:
                updated_prob_dist[binary_int] = non_flipped_prob
    ...
    def mld_contraction_no_slicing(self, syndrome: np.ndarray[bool]) -> Dict[
        int, Union[np.float32, np.float64]]:
        """
        ...
        Returns:
            Dict[int, Union[np.float32, np.float64]]: The resulting probability distribution after the contraction.
        """
    ...
    def mld_contraction_serial(self, syndrome: np.ndarray[bool]) -> Dict[int, Union[np.float32, np.float64]]:
        """
        ...
        Returns:
            Dict[int, Union[np.float32, np.float64]]: A dictionary representing the probability distribution after contraction.
    ...
    def mld_contraction_parallel_concurrent(
        ...
        max_thread: int = 4
    ) -> Dict[int, Union[np.float32, np.float64]]:
        """
        ...
        Returns:
            Dict[int, Union[np.float32, np.float64]]: The merged probability distribution after contraction.
    ...
    def single_node_online_mld_contraction(
            self,
            syndrome: np.ndarray[bool],
            init_prob_dist: Dict[Tuple[bool, ...], Union[np.float32, np.float64]],
        ...
        Args:
            ...
            init_prob_dist (Dict[Tuple[bool, ...], Union[np.float32, np.float64]]): Initial probability distribution.
        ...
        Returns:
            Dict[int, Union[np.float32, np.float64]]: The updated probability distribution for the given syndrome.
        ...
        for contraction_step in range(self.detector_number):
            ...
            prob_dist = {
                ...
                if check_bit_value(candidate_syndrome, contract_detector_index, self.total_length, observed_detector_syndrome)
            }
    ...
    @classmethod
    def validate_logical_operator(cls, prob_dist: Dict[int, Union[np.float32, np.float64]], logical_operators: str = 'X') -> Tuple[np.ndarray[bool], Union[np.float32, np.float64]]:
        """
        ...
        Args:
            prob_dist (Dict[int, Union[np.float32, np.float64]]): The resulting probability distribution after contraction.
