...
# @numba.njit
# def process_keys_numba(keys, values, contract_detector_index, p_00, p_11):
#     new_keys = np.empty_like(keys)
#     new_values = np.empty_like(values)
    
#     for i in range(len(keys)):
#         key = keys[i].copy()
#         bit = key[contract_detector_index]
#         rand_val = np.random.random()
        
#         if bit == '0' and rand_val > p_00:
#             key[contract_detector_index] = '1'
#         elif bit == '1' and rand_val > p_11:
#             key[contract_detector_index] = '0'
...
def build_hyperedge_contraction_caches(contractable_hyperedges_weights_dict: Dict[str, Union[np.float32, np.float64]], order: List[str]):
    """构建用于收缩的缓存构建方法"""
...
logger = setup_logger("contraction_executor/biased_approx_contraction_executor_qldpc", log_level=logging.WARNING)
...
class BiasedApproximateContractionExecutorQldpc:
    def __init__(self, detector_number: int, logical_number: int, order: List[str],
                 ...
                 priority: int = 1, priority_topk: int = 10, p_00:float = 1.0, p_11 = 1.0):
        """
    ...
    def flip_prob_dist(self, prob_dist: Dict[str, np.floating], contract_detector_index: int) -> Dict[str, np.floating]:
        if not prob_dist:
            return {}

        # 准备数据
        keys = np.array([list(k) for k in prob_dist.keys()])  # 二维字符数组
        values = np.array(list(prob_dist.values()))
        bits = keys[:, contract_detector_index]

        # 计算保留和翻转概率
        p_keep = np.where(bits == '0', self.p_00, self.p_11)
        p_flip = 1 - p_keep

        # 计算原始和翻转后的值
        original_values = values * p_keep
        flipped_values = values * p_flip

        # 生成翻转后的keys
        flipped_keys = keys.copy()
        flipped_keys[:, contract_detector_index] = np.where(bits == '0', '1', '0')

        # 合并所有keys和values
        all_keys = np.concatenate([keys, flipped_keys])
        all_values = np.concatenate([original_values, flipped_values])
        all_key_strs = np.array([''.join(k) for k in all_keys])

        # 使用np.add.at累加相同键的值
        unique_keys, indices = np.unique(all_key_strs, return_inverse=True)
        result_values = np.zeros(len(unique_keys), dtype=np.float64)
        np.add.at(result_values, indices, all_values)  # 关键修改：使用add.at代替maximum.at

        # 构造结果字典（保持原始数值类型）
        result_dict = {k: type(values[0])(v) for k, v in zip(unique_keys, result_values)}
        return result_dict
    ...
    def _priority_sub_hypergraph_contraction_strategy(self):
        ...
        sub_contractable_hyperedges_weights_dict: dict[str, float] = {
            ",".join(key): float(value) for key, value in original_dict.items()
        }
    ...
    def approximate_distribution(self, updated_prob_dist: Dict[str, Union[np.float32, np.float64]],) -> Dict[str, Union[np.float32, np.float64]]:
        ...
        if self._is_topk:
            ...
            return dict(nlargest(approximate_param, updated_prob_dist.items(), key=lambda item: item[1]))
    ...
    def single_node_online_mld_contraction(
        ...
        for contraction_step in range(contraction_step_num):
            ...
            prob_dist = self.flip_prob_dist(prob_dist, contract_detector_index)
    ...
    # @classmethod
    def validate_logical_operator(self, prob_dist: Dict[str, Union[np.float32, np.float64]]) -> Tuple[np.ndarray[bool], Union[np.float32, np.float64]]:
        """
    ...
    #     The prob is the cost, not probability.
    #     """
    #     # Get the probability of the contracted hyperedge
    #     contracted_hyperedge_prob = contractable_hyperedges_weights_dict.pop(contracted_hyperedge)
    #     non_flip_contracted_hyperedge_prob = (self.const_1 - contracted_hyperedge_prob)
    ...
    #     # Iterate over the current probability distribution
    #     for before_int, prob in prob_dist.items():
    #         # Flip the bits for the current hyperedge
    #         flipped_int = self.flip_bits(before_int, contracted_hyperedge)
            
    #         # Calculate the updated probabilities based on the hyperedge contraction
    #         # w(e) = -log(p)/log(1-p), so 都发生，则w(e_1)+w(e_2)，不发生，则不进行任何操作。
    #         flip_cost = contracted_hyperedge_prob + prob
    #         non_flipped_cost = prob
    #         # flipped_prob = prob * contracted_hyperedge_prob
    #         # non_flipped_prob = prob * non_flip_contracted_hyperedge_prob
    ...
    #         if flipped_int in updated_prob_dist:
    #             # 处理成取最大值
    #             if flip_cost > updated_prob_dist[flipped_int]:
    #                 updated_prob_dist[flipped_int] = flip_cost
    #         else:
    #             updated_prob_dist[flipped_int] = flip_cost

    #         if before_int in updated_prob_dist:
    #             # 处理成取最大值
    #             if non_flipped_cost > updated_prob_dist[before_int]:
    #                 updated_prob_dist[before_int] = non_flipped_cost
    #             # updated_prob_dist[before_int] += non_flipped_cost
    #         else:
    #             updated_prob_dist[before_int] = non_flipped_cost
