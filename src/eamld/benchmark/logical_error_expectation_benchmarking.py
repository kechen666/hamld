...
logger = setup_logger("eamld/benchmark/logical_error_expectation_benchmarking", log_level=logging.WARNING)

class LogicalErrorExpectationBenchmark:
    def __init__(self, decoder_function, d, r, p, noise_model, error_type="Z", have_stabilizer = True):
        """
        初始化期望值基准测试对象。
        
        该方法只支持MLD类的方法，比如MLD和EAMLD。
    ...
    def logical_error_expectation(self, syndrome: np.ndarray) -> np.float64:
        """
        计算逻辑错误期望值。
        ...
        if prob_dists is not None:
            # 初始化概率累加器
            total_prob = np.float64(0)
            true_prob = np.float64(0)
            error_prob = np.float64(0)
            ...
            for i in range(num_shots):
                ...
                if isinstance(np.bool_(True), (bool, np.bool_)):
                    if predicted_value:
                        # 预测值为 True 时，累加相应概率
                        true_prob += np.float64(prob_dists[i][1])
                        error_prob += np.float64(prob_dists[i][0])
                    else:
                        # 预测值为 False 时，累加相应概率
                        true_prob += np.float64(prob_dists[i][0])
                        error_prob += np.float64(prob_dists[i][1])
                else:
                    # 若预测值不是布尔类型，抛出异常
                    raise ValueError(f"预测观测量中索引 {i} 处出现意外值: {predicted_value}")

                # 累加当前样本的总概率
                total_prob += np.float64(prob_dists[i][0]) + np.float64(prob_dists[i][1])
    ...
    def run(self):
        ...
        syndrome = generate_all_possible_syndromes(self.d, self.r, self.have_stabilizer)
