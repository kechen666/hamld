...
logger = setup_logger("eamld/benchmark/logical_error_rate_benchmarking", log_level=logging.INFO)
...
class LogicalErrorRateBenchmark:
    def __init__(self, decoder_function, d = None , nkd =  None, r = 1, p = 10, noise_model = "si1000", error_type="Z", num_runs=1, 
                 data_path="../data/external/eamld_experiment_data/experiment_1_surface_code_fault_tolerance_threshold",
                 code_name = "surface code", have_stabilizer = False, is_biased = False, biased_measure_param = 1):
        """
        初始化基准测试对象。
    ...
    def run(self):
        ...
        if self.code_name == "surface code":
            syndrome, actual_observables = generate_syndrome_and_observables(self.d, self.r, self.p, self.noise_model, 
                                                                             ...
                                                                             self.is_biased, self.biased_measure_param)
        ...
        #     syndrome = postprocess_syndrome(syndrome, p_00, p_11)
        
        for _ in range(self.num_runs):
            start_time = time.time()  # 记录开始时间
            
            # 计算逻辑错误率
            error_rate = self.logical_error_rate(syndrome, actual_observables)
