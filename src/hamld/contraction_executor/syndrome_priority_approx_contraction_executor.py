...
def str_to_binary_bitwise(s: str) -> np.ndarray[bool]:
    """
    将二进制字符串转换为布尔数组
    :param s: 二进制字符串，例如 "1010"
    :return: 布尔元组，例如 (True, False, True, False)
...
# def hamming_distance_numpy(s1: np.ndarray[bool], s2: str):
...
#     )
def sparse_hamming_distance_vec(s1: np.ndarray[bool], s2: str) -> int:
    len1 = len(s1)
    len2 = len(s2)

    # 对齐 s2 并转换为布尔 numpy 数组（True 表示 '1'）
    if len2 < len1:
        s2 += '0' * (len1 - len2)
    elif len2 > len1:
        s2 = s2[:len1]

    s2_bool = np.fromiter((c == '1' for c in s2), dtype=bool, count=len1)

    # 向量化 XOR，然后统计不同的位数（即汉明距离）
    return np.count_nonzero(s1 ^ s2_bool)
...
def compute_score(v, hamming_weight, beta=1.0):
    if v <= 0.0:
        return float('inf')  # 越差越大，优先级低
    if v >= 1.0:
        return -float('inf')  # 越好越小，优先级高
    return -np.log(v / (1 - v)) * (hamming_weight + beta)

# 基于new_priority_approx_contraction_executor_qldpc.py函数进行修改
class SyndromePriorityApproximateContractionExecutor:
    def __init__(self, detector_number: int, logical_number: int, order: List[str],
                 ...
                 gamma = 23, contract_logical_hyperedges: bool = True):
        """
    ...
    def approximate_distribution(self, updated_prob_dist: Dict[str, Union[np.float32, np.float64, np.float128]], syndrome: np.ndarray[bool]) -> Dict[str, Union[np.float32, np.float64, np.float128]]:
        """
        ...
        if self._is_topk:
            ...
            # 计算score值，为log(v)/log(1-v) * |str_syndrome \oplus k|相关
            scored_items = [
                (compute_score(v, sparse_hamming_distance_vec(syndrome, k)), k, v)
                for k, v in updated_prob_dist.items()
            ]
            # 使用 nsmallest 加速筛选（注意：这里是选择“最小 score”）
            top_items = heapq.nsmallest(approximate_param, scored_items)

            return {k:v for _, k, v in top_items}
    ...
    def contract_hyperedge(
        ...
        Args:
            ...
            syndrome (np.array): The syndrome of the contracted hyperedge to approximation contraction.
        ...
        if self._approximate_position == "hyperedge":
            updated_prob_dist = self.approximate_distribution(updated_prob_dist, syndrome)
        ...
        for contraction_step in range(contraction_step_num):
            ...
            for hyperedge in relevant_hyperedges:
                ...
                prob_dist, contractable_hyperedges_weights = self.contract_hyperedge(
                    prob_dist, contractable_hyperedges_weights, hyperedge, syndrome
                )
            ...
            if self._approximate_position == "node":
                ...
                prob_dist = self.approximate_distribution(prob_dist, syndrome)
