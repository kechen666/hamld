...
def process_syndrome(sample_syndrome, shots):
    """
    直接从原始 syndrome 数据中随机选择 shots 个样本。

    参数：
        - sample_syndrome: 原始 syndrome 数据 (shape: [num_samples, syndrome_length])
        - shots: 保留的采样次数
    返回：
        - selected_syndrome: 随机选择的 syndrome 数据 (shape: [shots, syndrome_length])
    """
    num_samples = sample_syndrome.shape[0]
    len_syndrome = sample_syndrome.shape[1]

    if num_samples > shots:
        selected_indices = np.random.choice(num_samples, shots, replace=False)
        selected_syndrome = sample_syndrome[selected_indices, :len_syndrome]
    else:
        selected_syndrome = sample_syndrome[:, :len_syndrome]

    return selected_syndrome


logger = setup_logger("hamld/benchmark/decoder_speed_benchmarking", log_level=logging.INFO)
...
class DecoderSpeedBenchmark:
    def __init__(self, decoder_function, d = None , nkd =  None, r = 1, p = 10, noise_model = "si1000", error_type="Z", num_runs=10, 
                 ...
                 data_path = None, have_stabilizer = False, code_language = "python"):
        """
        初始化基准测试对象。
    ...
    def decoder_per_time(self, syndrome: np.ndarray) -> float:
        ...
        if self.code_language == "python":
            ...
            # self.decoder_function.decode_batch_parallel(syndrome, num_processes = 128)
            # print("all time:", self.decoder_function.total_decode_time)
            # except:
            #     logger.info("no parallel")
            self.decoder_function.decode_batch(syndrome)
        elif self.code_language == "c++":
            self.decoder_function.decode_batch(syndrome, False)

        per_shots_time = (time.time() - start_time) / num_shots
        
        # per_rounds_time = per_shots_time / self.r
        per_syndrome_time = per_shots_time
        
        return per_syndrome_time
    ...
    def run(self, shots):
        ...
        average_per_syndrome_time = np.mean(per_time_list)
        
        logger.info(f"Average decode time (over {self.num_runs} runs), rounds is {self.r}: per syndrome time: {average_per_syndrome_time:.6f}")
        
        return average_per_syndrome_time, per_time_list
