...
def generate_syndrome_and_observables(d: int, r: int, p: float, noise_model: str, error_type: str, 
                                       related_path: str = "../data/external/hamld_experiment_data/experiment_1_surface_code_fault_tolerance_threshold", 
                                       have_stabilizer = True, is_bias = False, bias_param = None, measurement_param = None) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    # 构建文件路径
    base_dir = pathlib.Path(related_path) / f"{error_type}/d{d}_r{r}"
    suffix = f"_p{p}"
    if measurement_param is not None:
        suffix += f"_m{measurement_param}"
    if not have_stabilizer:
        suffix += "_no_stabilizer"
    if is_bias and bias_param is not None:
        suffix += f"_bias{bias_param}"

    file_path = base_dir / f"circuit_sample_{noise_model}{suffix}.dat"
    # decomposed_file_path = base_dir / f"decomposed_detector_error_model_{noise_model}{suffix}.dem"
...
def generate_qldpc_syndrome_and_observables(nkd: List[int], r: int, p: float = None, noise_model: str = None, error_type: str = "Z", 
                                       related_path: str = "../data/external/hamld_experiment_data/paper_experiment_bb_codes", 
                                       have_stabilizer = False, is_bias = False, bias_param = None, measurement_param = None, file_path = None) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    if file_path is None:
        # 构建文件路径
        base_dir = pathlib.Path(related_path) / f"{error_type}/nkd_{nkd[0]}_{nkd[1]}_{nkd[2]}_r{r}"
        suffix = f"_p{p}"
        if measurement_param is not None:
            suffix += f"_m{measurement_param}"
        if not have_stabilizer:
...
def generate_detector_error_model(d: int, r: int, p: float, noise_model: str, error_type: str, 
                                  ...
                                  measurement_param = None) -> stim.DetectorErrorModel:
    """
    生成对应的探测器错误模型。
    ...
    - related_path: 数据文件路径，注意最后要加/。
    - measurement_param: 测量噪声参数。
    
    返回：
    - 返回 `stim.DetectorErrorModel` 对象，表示探测器错误模型。
    """
    # 构建文件路径
    # if measurement_param is None:
    #     if have_stabilizer:
    ...
    # else:
    #     if have_stabilizer:
    #         file_path = pathlib.Path(related_path + "/" + f'{error_type}/d{d}_r{r}/detector_error_model_{noise_model}_p{p}_m{measurement_param}.dem')
    #         decomposed_file_path = pathlib.Path(related_path + "/" + f'{error_type}/d{d}_r{r}/decomposed_detector_error_model_{noise_model}_p{p}_m{measurement_param}.dem')
    #     else:
    #         # 构建文件路径
    #         file_path = pathlib.Path(related_path + "/" + f'{error_type}/d{d}_r{r}/detector_error_model_{noise_model}_p{p}_m{measurement_param}_no_stabilizer.dem')
    #         decomposed_file_path = pathlib.Path(related_path + "/" + f'{error_type}/d{d}_r{r}/decomposed_detector_error_model_{noise_model}_p{p}_m{measurement_param}_no_stabilizer.dem')
    ...
    file_path = base_dir / f"detector_error_model_{noise_model}{suffix}.dem"
    decomposed_file_path = base_dir / f"decomposed_detector_error_model_{noise_model}{suffix}.dem"
...
def generate_detector_error_model_path(d: int, r: int, p: float, noise_model: str, error_type: str, 
                                  decomposed_error: bool = False, 
                                  related_path: str = "../data/external/hamld_experiment_data/experiment_1_surface_code_fault_tolerance_threshold",
                                  have_stabilizer = True,
                                  measurement_param = None) -> stim.DetectorErrorModel:
...
def generate_qldpc_detector_error_model_path(nkd: List[int], r: int, p: float, noise_model: str, error_type: str,
                                  related_path: str = "../data/external/hamld_experiment_data/paper_experiment_bb_codes", 
                                  have_stabilizer = True,
                                  measurement_param = None) -> str:
    """
    生成对应的探测器错误模型的路径。
...
def generate_all_possible_syndromes(d: int, r: int, have_stabilizer: bool = True):
    if have_stabilizer:
        syndrome_length = (d**2 - 1) * r
    else:
        syndrome_length = int((d**2 - 1) * (r-1) +  (d**2 - 1)/2)
...
def generate_syndromes_generator(d: int, r: int):
    # 生成器的形式，可以通过遍历逐步生成。
    syndrome_length = (d**2 - 1) * r
