...
class DetectorErrorModelHypergraph:
    ...
    def get_new_priority_sub_hypergraph(self, nodes: List[str], priority: int, topk: Union[int, None] = None) -> "DetectorErrorModelHypergraph":
        ...
        # 计算每条超边的priority(连接nodes的数量)和权重
        # 预过滤只包含D节点的nodes
        d_nodes_set = {node for node in nodes_set if node.startswith("D")}
        
        filpped_node_number = len(d_nodes_set)
        
        # log-priority：用于初筛
        log_priority = max(-1, int(np.log10(filpped_node_number))-1)
        # if filpped_node_number >=40:
        #     log_priority = 0
        # elif filpped_node_number>=30:
        #     log_priority = -1
        # else:
        #     log_priority = -2
        ...
        for edge, weight in zip(self.hyperedges, self.weights):
            # 获取当前边中的所有D节点
            edge_d_nodes = {node for node in edge if node.startswith("D")}
            
            # 如果超边不包含任何D节点，则跳过，不考虑全为逻辑错误的情况，对于解码没有用。
            if not edge_d_nodes:
                continue
            
            match_number = len(d_nodes_set & edge_d_nodes)
            mismatch_number = len(edge_d_nodes - d_nodes_set)
            
            # 跳过完全没有匹配的内容。
            if match_number == 0 and mismatch_number>2:
                continue

            # # 想法3：如果边全部匹配 syndrome 中的 D 节点，则强制保留。同时优先级设置为最高，翻转比特数+匹配数。
            if mismatch_number == 0 or filpped_node_number == match_number:
                edge_priority = filpped_node_number + match_number
                edge_info.append((edge, weight, edge_priority))
                continue
            
            # # 想法：mismatch_number大于某个值，也直接跳过，这里假设为2：
            # if mismatch_number > 3:
            #     continue
            
            # 计算priority
            edge_priority = match_number - mismatch_number
            
            if edge_priority >= log_priority:
                edge_info.append((edge, weight, edge_priority))
        ...
        if topk is not None:
            # 动态的topk
            # MAX_EDGE_COUNT = min(10 * filpped_node_number, topk)
            # edge_info = edge_info[:MAX_EDGE_COUNT]
            edge_info = edge_info[:topk]
        ...
        # # 获取子超图中实际存在的节点(超边中出现的节点)
        # 获取子超图中实际存在的节点(使用集合操作)
        actual_nodes = set().union(*sub_hyperedges)
